...
import { type Instructor, type InstructorUpdateDto, instructorService } from "./services/instructor.service"
...
export const fetchInstructors = createAsyncThunk("instructors/fetchAll", async (_, { rejectWithValue }) => {
  try {
...
export const createInstructor = createAsyncThunk(
  ...
  async (instructor: Omit<Instructor, "id">, { rejectWithValue }) => {
    try {
      ...
      const missingFields = requiredFields.filter((field) => !instructor[field as keyof typeof instructor])
...
export const deleteInstructor = createAsyncThunk("instructors/delete", async (id: string, { rejectWithValue }) => {
  try {
